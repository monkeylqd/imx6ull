...
#if 1
	// skip interfaces 4 used as USB network device above for Quectel some devices
	switch (serial->dev->descriptor.idVendor) {
	case cpu_to_le16(0x05C6):
		if (unlikely((serial->dev->descriptor.idProduct == cpu_to_le16(0x9003) ||
		    serial->dev->descriptor.idProduct == cpu_to_le16(0x9215)) &&
			serial->interface->cur_altsetting->desc.bInterfaceNumber >= 4))
				return -ENODEV;
		break;
	case cpu_to_le16(0x2C7C):
		if (unlikely(serial->interface->cur_altsetting->desc.bInterfaceNumber >= 4))
			return -ENODEV;
		break;
	default:
		break;
